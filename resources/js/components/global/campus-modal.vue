<template>
	<div>
		...
		<v-dialog v-model="showModal" persistent scrollable aria-modal="true" @click:outside="handleOutsideClick">
...
export default {
	...
	created() {
		this.boundEscHandler = this.onEscKey.bind(this);
		document.addEventListener("keyup", this.boundEscHandler);
	...
	beforeUnmount() {
		document.removeEventListener("keyup", this.boundEscHandler);
	},
	computed: {
		...mapGetters([
			"universities",
