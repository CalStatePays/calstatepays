			...
			<div class="col-md-9">
				<card v-if="selectedFormWasSubmittedOnce" class="csu-card container-fluid">
					...
					<div
						...
						:class="['float-right', { invisible: nullValues }]"
					>
						<a
							class="csu-card__share csu-card__share-twitter float-right"
						...
						<a
							class="csu-card__share csu-card__share-linkedin float-right"
						...
						<a
							class="csu-card__share csu-card__share-facebook float-right"
...
export default {
	...
	computed: {
		...
		shareLinks() {
			const shareUrl = encodeURIComponent(window.location.href || this.url);
			...
			return {
				twitter: `https://twitter.com/intent/tweet?url=${shareUrl}&text=${message}&hashtags=${hashtags}`,
				linkedin: `https://www.linkedin.com/shareArticle?mini=true&url=${shareUrl}&title=${message}&summary=${message}`,
