			...
			<div v-else>
					...
					<div class="container-fluid py-3">
						<div class="row">
							...
							<div class="col-11">
					<div v-if="selectedFormWasSubmitted" :class="{ invisible: nullValues }">
						<a class="csu-card__share csu-card__share-twitter float-right" :href="shareLinks.twitter" target="_blank" rel="noopener">
							<i class="fa fa-twitter-square"></i>
							Tweet
						</a>
						<a class="csu-card__share csu-card__share-linkedin float-right" :href="shareLinks.linkedin" target="_blank" rel="noopener">
							<i class="fa fa-linkedin-square"></i>
							Share
						</a>
						<a class="csu-card__share csu-card__share-facebook float-right" :href="shareLinks.facebook" target="_blank" rel="noopener">
...
<script>
	...
	data() {
		...
		shareLinks() {
			const current = window.location?.href || window.baseUrl;
			const baseUrl = encodeURIComponent(current);
			const message = encodeURIComponent(this.shareDescription || 'Discover your earnings after college!');
			const hashtags = encodeURIComponent('CalStatePays,ItPaysToGoToCollege');
			return {
				twitter: `https://twitter.com/intent/tweet?url=${baseUrl}&text=${message}&hashtags=${hashtags}`,
				linkedin: `https://www.linkedin.com/shareArticle?mini=true&url=${baseUrl}&title=${message}&summary=${message}`,
